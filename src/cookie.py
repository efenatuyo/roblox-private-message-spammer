...
class RobloxCookie(Bypass):
    ...
    async def generate_token(self, session, proxy):
        self._x_token = (await session.post("https://auth.roblox.com/v2/logout", cookies={".ROBLOSECURITY": self.cookie}, proxy=proxy)).headers.get("x-csrf-token")
        self.last_generated_time = time.time()

    async def x_token(self, session, proxy):
        current_time = time.time()
        if current_time - self.last_generated_time >= 120:
            await self.generate_token(session, proxy)
